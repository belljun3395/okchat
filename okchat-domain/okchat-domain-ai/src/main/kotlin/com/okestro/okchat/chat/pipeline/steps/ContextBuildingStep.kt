...
class ContextBuildingStep(
    ...
    override suspend fun execute(context: ChatContext): ChatContext {
        ...
        val highRelevance = validResults.filter { it.score >= HIGH_RELEVANCE_THRESHOLD }
        val mediumRelevance = validResults.filter { it.score >= MEDIUM_RELEVANCE_THRESHOLD && it.score < HIGH_RELEVANCE_THRESHOLD }
        val otherResults = validResults.filter { it.score < MEDIUM_RELEVANCE_THRESHOLD }
    ...
    private fun StringBuilder.appendDocumentInfo(index: Int, result: SearchResult, detailed: Boolean) {
        ...
        append("   Relevance: ${"%.2f".format(result.score)}")
    ...
    private fun logTopResults(topResults: List<SearchResult>) {
        ...
        topResults.forEachIndexed { index, result ->
            ...
            log.debug { "Score: ${"%.4f".format(result.score)}, ID: ${result.id}" }
