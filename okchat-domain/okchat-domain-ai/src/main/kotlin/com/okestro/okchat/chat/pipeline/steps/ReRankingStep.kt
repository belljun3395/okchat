    ...
    private fun logReRankedResults(reranked: List<SearchResult>) {
        if (log.isDebugEnabled()) {
            ...
            reranked.forEachIndexed { index, result ->
                log.debug { "  [${index + 1}] ${result.title} (score: ${"%.4f".format(result.score)}, id: ${result.id}, content: ${result.content.length} chars)" }
            }
            log.debug { "[${getStepName()}] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" }
        } else {
            log.info {
                "[${getStepName()}] Top 5: ${
                reranked.take(5).joinToString(", ") { "${it.title}(${"%.4f".format(it.score)})" }
                }"
            }
        }
    }
