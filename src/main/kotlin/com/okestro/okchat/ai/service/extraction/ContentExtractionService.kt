    ...
    override fun buildPrompt(message: String): Prompt {
        ...
        val examples = listOf(
            PromptExample(
                ...
                output = "{\"keywords\": [\"new authentication logic\", \"authentication\", \"logic\", \"PPP project\"]}"
            ),
            PromptExample(
                input = "How is the memory leak in the notification service being handled?",
                output = "{\"keywords\": [\"memory leak\", \"notification service\"]}"
            ),
            PromptExample(
                input = "성능 테스트 결과 보고서에서 병목 현상에 대한 내용 찾아줘",
                output = "{\"keywords\": [\"병목 현상\", \"성능 테스트\"]}"
            ),
            PromptExample(
                input = "I want to know the agenda for the next weekly meeting.",
                output = "{\"keywords\": [\"weekly meeting\", \"agenda\"]}"
            )
        )
